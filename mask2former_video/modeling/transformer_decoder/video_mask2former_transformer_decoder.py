    ...
    def _load_from_state_dict(
        ...
        if version is None or version < 2:
            ...
            for k in list(state_dict.keys()):
                ...
                if "seg_decoder" in prefix:
                    if "predictor" in k:
                        newk = k.replace("predictor", "seg_decoder")
                # depth init with seg_weight
                if "depth_decoder" in prefix:
                    if "seg_decoder" in k:
                        newk = k.replace("seg_decoder", "depth_decoder")
                    if "mask_embed" in k:
                        newk = newk.replace("mask_embed", "depth_embed")
                    state_dict.update({newk: state_dict[k]})
                    scratch = False
                if "unified_decoder" in prefix:
                    if "query" in k:
                        newk = newk.replace("query", "unified_query")
                    ##### depth_embed init with seg_mask_embed
                    # if "mask_embed" in k:
                    #     newk = newk.replace("mask_embed", "depth_embed")
