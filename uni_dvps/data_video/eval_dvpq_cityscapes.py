...
def eval(element):
    ...
    if depth_thres > 0:
        ...
        for depth_pred, depth_gt, pred in zip(depth_preds, depth_gts, preds):
            ...
            depth_pred_div = depth_pred / 256
            depth_gt_div = depth_gt / 256
            depth_mask_div = depth_gt_div > 0.
...
def main(eval_frames, pred_dir, gt_dir, depth_thres):
    ...
    sq = iou_per_class / (tp_per_class)
    rq = tp_per_class / (tp_per_class +
                         0.5 * fn_per_class + 0.5 * fp_per_class)
...
def _print_panoptic_results(eval_frames, depth_thres, pq_result):
    ...
    for name in ["All", "Things", "Stuff"]:
        if name == "All":
            ...
            row = [name] + [pq_result[k][11:].mean() * 100 for k in ["pq", "sq", "rq"]] + [len(pq_result["pq"][11:])]
        elif name == "Stuff":
            row = [name] + [pq_result[k][:11].mean() * 100 for k in ["pq", "sq", "rq"]] + [len(pq_result["pq"][:11])]
        data.append(row)
    table = tabulate(
